...
🔄 Обновление TelegramMaster-PRO 🚀<br>
📦 Версия: 2.9.0<br>
📅 Дата релиза: 09.09.2026<br>

1. Нормализация ссылок Telegram с поддержкой ссылок на конкретные посты (/1448) и страниц веб-просмотра (/s/).
2. Автоматическая очистка ссылок перед подпиской и перед отправкой сообщений в циклах рассылки.
3. Настраиваемый перерыв между кругами рассылки по всем группам с отображением времени и прогресс-бара.
4. Безопасная остановка процесса ожидания между кругами рассылки при отключении или нажатии «Стоп».
5. Обновление и стилизация компонентов GUI.
...
🔄 Обновление TelegramMaster-PRO 🚀<br>
📦 Версия: 2.9.1<br>
📅 Дата релиза: 16.09.2026<br>

1. Реализован пользовательский выбор темы оформления (светлая ☀️ / тёмная 🌙) в меню настроек.
2. Добавлены отдельные интерактивные кнопки управления темой с визуальным откликом.
3. Сохранение выбранного режима темы в `user_data/config.ini` и автоматическая загрузка при старте приложения.
4. Исправлена обработка конфигурационного файла `ConfigParser` для корректного чтения параметров с пустыми значениями.
...
🔄 Обновление TelegramMaster-PRO 🚀<br>
📦 Версия: 2.9.2<br>
📅 Дата релиза: 18.09.2026<br>

1. Реализована функция подключения аккаунтов Telegram по QR-коду.
2. Добавлена генерация векторных QR-кодов в оперативной памяти (без создания временных файлов на диске).
3. Автоматическое сохранение сессий под именем номера телефона пользователя и добавление в базу данных.
4. Поддержка двухфакторной аутентификации (2FA) при авторизации по QR-коду.
...
🔄 Обновление TelegramMaster-PRO 🚀<br>
📦 Версия: 2.9.3<br>
📅 Дата релиза: 25.09.2026<br>

1. ⚡ Реализована автоматическая асинхронная проверка версий через GitHub REST API.
2. 🔴🟢 Добавлена интеллектуальная плашка на главной странице: при выходе нового релиза ваша версия подсвечивается красным, доступная — зеленым, с прямой ссылкой на Telegram-канал для скачивания.
3. 🛠️ Исправлена задержка и пакетный вывод сообщений логов в GUI интерфейсе (`AppLogger.log_and_display` с вызовом `self.list_view.update()`).
4. ⏱️ Все блокирующие `time.sleep()` заменены на асинхронные `await asyncio.sleep()`.
5. 🎨 Обновлены тексты и иконки кнопок в меню рассылки и настройках («✉️ Создать сообщение для рассылки», «▶️ Начать рассылку»).
