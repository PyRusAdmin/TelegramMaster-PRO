...
1. Удалено излишнее логирование, чтобы пользователя не перегружать информацией о работе программы, за исключением записи
   ошибок.
2. Функции «Рассылка сообщений в личку» и «Рассылка сообщений по чатам» вынесены на главное меню в виде отдельных кнопок
...
6. Добавлена проверка на дублирующиеся записи при использовании функции «📑 Формирование списка чатов / каналов» при
   записи в чистую базу данных.
7. В функции «🔍 Парсинг одной группы / групп» добавлены CupertinoSwitch-переключатели для удобного выбора режима:
   парсинг администраторов, парсинг групп, на которые подписан аккаунт, или парсинг участников по ранее сформированному
   списку.
...
19. В меню «Инвайтинг» добавлен прямой ввод ссылок для приглашений, что упрощает процесс и улучшает пользовательский
    опыт.
20. В меню «Подписка / Отписка» добавлен ввод ссылок на группы и каналы. Ссылки сохраняются в базу данных в таблицу
    writing_group_links для последующей обработки.
...
24. Из настроек программы удалён параметр ввода времени запуска инвайтинга в определённое время, так как он перенесён в
    соответствующее меню для улучшения интерфейса.
25. В меню «Инвайтинг» добавлен ввод лимитов на аккаунт, что позволяет гибко настраивать параметры без перехода в
    настройки программы.
26. Добавлен ввод времени задержки (сна) между приглашениями в инвайтинге для соблюдения лимитов Telegram.
27. В меню «Инвайтинг» добавлены CupertinoSwitch-переключатели для простого переключения между режимами: инвайтинг 1 раз
    в час, в определённое время или ежедневно.
...
31. Из настроек программы удалены устаревшие параметры: Запись времени, Запись ссылки, Лимиты на аккаунт, Лимиты на
    сообщения, Формирование списка username, Время между подпиской, Запись времени между сообщениями, Время между
    инвайтингом, Рассылка сообщений, Формирование списка чатов / каналов, Очистка сформированного списка чатов.
...
38. В функции «️‍🗨️ Накручиваем просмотры постов» добавлено отображение времени начала и завершения накрутки, а также
    ввод количества просмотров. При наличии 10 аккаунтов пользователь может указать до 10 просмотров.
